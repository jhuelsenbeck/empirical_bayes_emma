...
// Custom hash function for well-distributed uint64_t keys
struct FastHash {
    size_t operator()(uint64_t key) const {
        // Identity function for well-distributed uint64_t hashes
        return static_cast<size_t>(key);
    }
};

typedef std::unordered_map<uint64_t, TreeInfo, FastHash> TreeMap;
