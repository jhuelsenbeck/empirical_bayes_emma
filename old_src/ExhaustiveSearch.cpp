...
ExhaustiveSearch::ExhaustiveSearch(Alignment* aln, TreeList* tl, ThreadPool* tp) : 
    alignment(aln), treeList(tl), threadPool(tp), numTrees(0), scratchRoot(nullptr) {
...
void ExhaustiveSearch::enumerateAllTrees(void) {
    ...
    size_t numTrees = treeList->size();
    size_t maxJobs = threadPool->getQueueCapacity();
    if (numTrees < maxJobs)
        maxJobs = treeList->size();
