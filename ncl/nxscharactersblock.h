...
/*!
	Assigns the gap symbol and updates the gap symbol of the single datatype mapper if only one is present.
*/
inline void NxsCharactersBlock::SetGapSymbol(char g)
	{
	gap = g;
	if (datatypeMapperVec.size() == 1 && datatypeMapperVec[0].first)
...
// Insert moved inline definitions here after all above
