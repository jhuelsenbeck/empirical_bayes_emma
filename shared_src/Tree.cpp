...
Tree::Tree(CompactTree* ct, int nt) {

    int    taxonBits = CompactTree::bitsPerIndex(static_cast<int>(nt));
    Node*  p         = nullptr;
    numTips          = 0;
    size_t pos       = 0;
    numNodes = 0;
    ...
    while (true)
        {
        CompactTree::Code code = ct->readCode(pos);

        if (code == CompactTree::Code::End)
            break;

        if (code == CompactTree::Code::Open)
            {
            Node* newNode = addNode();
            newNode->setBrlen(-1.0);
            if (p == nullptr)
        ...
        else if (code == CompactTree::Code::Close)
            {
            if (p->getAncestor() == nullptr)
                Msg::error("Cannot move down tree when building Newick tree");
            p = p->getAncestor();
            ...
            {
            uint32_t idx = ct->readIndex(pos, taxonBits);
            Node* newNode = addNode();
            newNode->setIndex(static_cast<int>(idx));
    ...
    for (Node* p : downPassSequence)
        p->setBrlen(p == root ? 0.0 : 0.02);

    setDescriptor();
}

Tree::Tree(CompactTree* ct, std::vector<std::string>& taxonNames) {

    int    taxonBits = CompactTree::bitsPerIndex(static_cast<int>(taxonNames.size()));
    Node*  p         = nullptr;
    int    ntips     = 0;
    size_t pos       = 0;
    numNodes = 0;
...
CompactTree* Tree::getCompactRepresentation(void) {

    return new CompactTree(CompactTree::encode(this));
}
